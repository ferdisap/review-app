  ...
  <div x-data="{
     postCategory: function(draftOrPublished){
      ...
      init(){
        ...
        this.mousedownEvent();
        this.postCategory(document.querySelector('div[active]').getAttribute('active'));
        {{-- postCategory(document.querySelector('button[for={{ $active }}]')) --}}
      ...
      selectBox(e, executor){
        ...
        {{-- (sum = this.getCheckedQty())[1].length == sum[0].length ? this.saSwitch(false) : 
        (sum[1].length == 0 ? (() => {
          this.dsaSwitch();
      ...
      saSwitch(isSelectAllBox = true){
        document.querySelector('#toogle-switch').checked = true;
        isSelectAllBox ? this.selectAllBox(true) : null;
      },
      dsaSwitch(){
        document.querySelector('#toogle-switch').checked = false;
      },
      toogle(isSelectAll){
